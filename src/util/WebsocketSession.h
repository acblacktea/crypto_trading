...
namespace util::Websocket  {
    ...
    public:
        explicit Session(net::io_context &ioc, ssl::context &ctx, std::string host)
                : resolver_(net::make_strand(ioc)), ws(net::make_strand(ioc), ctx), host(std::move(host)) {
        }
        ...
        void onResolve(beast::error_code ec, const tcp::resolver::results_type &results) {
            ...
            beast::get_lowest_layer(ws).async_connect(results, beast::bind_front_handler(&Session::onConnect,
                                                                                         shared_from_this()));
        }
