...
// ((KLine, kLine, data))
/*
class KLineV2 : public JsonObject, Event
{
public:
    ...
    void setCloseTime(long long _closeTime)
    {
    ...
    void setFirstTradeID(long long _firstTradeID)
    {
        firstTradeID = _firstTradeID;
    }
    void setLastTradeID(long long _lastTradeID)
    {
        lastTradeID = _lastTradeID;
    }
    void setOpenPrice(std::string && _openPrice)
    {
        openPrice = std::move(_openPrice);
    }
    void setClosePrice(std::string && _closePrice)
    {
        closePrice = std::move(_closePrice);
    }
    void setHighPrice(std::string && _highPrice)
    {
        highPrice = std::move(_highPrice);
    }
    void setLowPrice(std::string && _lowPrice)
    {
        lowPrice = std::move(_lowPrice);
    }
    void setBaseAssetVolume(std::string && _baseAssetVolume)
    {
        baseAssetVolume = std::move(_baseAssetVolume);
    }
    void senNumberTrades(int _numberTrades)
    {
        numberTrades = _numberTrades;
    }
    void setIsKLineClosed(bool _isKLineClosed)
    {
        isKLineClosed = _isKLineClosed;
    }
    void setQuoteAssetVolume(std::string && _quoteAssetVolume)
    {
        quoteAssetVolume = std::move(_quoteAssetVolume);
    }
    void setTakerBuyAssetVolume(std::string && _takerBuyAssetVolume)
    {
        takerBuyAssetVolume = std::move(_takerBuyAssetVolume);
    }
    void setTakerBuyQuoteVolume(std::string && _takerBuyQuoteVolume)
    {
        takerBuyQuoteVolume = std::move(_takerBuyQuoteVolume);
    }
};
...
class KLineEventV2 : public JsonObject, public Event
{
public:
    bool deserialize(const rapidjson::Value & obj) override;
...
private:
    void setEventType(std::string && _eventType)
    {
        eventType = std::move(_eventType);
    }

    void setEventTime(long long _eventTime)
    {
