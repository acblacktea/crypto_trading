...
namespace USDM
{
DEFINE_CLASS(CancelOrderRequestParams((std::string, apiKey, apiKey))((std::string, signature, signature))((std::string, symbol, symbol))(
    (long long, timestamp, timestamp))((std::optional<long long>, orderId, orderId))((std::optional<long long>, recvWindow, recvWindow)))

DEFINE_CLASS(CancelOrderRequest((std::string, method, method))((CancelOrderRequestParams, params, params)))


DEFINE_CLASS(CancelOrderResponseResult(((long long, orderId, orderId))((std::string, symbol, symbol))((std::string, status, status))(
