...
namespace Binance
{
class TestNewOrderRequest : public JsonObject
{
public:
    ...
    std::optional<bool> computeCommissionRates;
};

inline std::string TestNewOrderRequest::serialize()
{
    ...
    if (computeCommissionRates.has_value())
    {
        doc.AddMember("computeCommissionRates", computeCommissionRates.value(), doc.GetAllocator());
    }
...
inline bool TestNewOrderRequest::deserialize(const rapidjson::Value & obj)
{
    return false;
}
}